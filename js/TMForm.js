	...
	$.fn.TMForm=function(opt){
		...
		function TMForm(){
			...
			function fieldValidate(el){
				...
				if(el.hasClass('nuiNotRequire')) return;
			...
			function formSubmit(){
				...
				if(!$('label.'+opt.invalidClass+',label.'+opt.emptyClass,form).length&&!form.hasClass(opt.processingClass)){
					...
					$this.ajaxSubmit(function(e,d,a,c){
						responseMessage=e;
						if(e.substring(0,15)=='<!DOCTYPE html>'){							
							form
								.removeClass(opt.processingClass)
								.addClass(opt.successClass)

							modal.find('.modal-title').text('Thành công!')
							modal.find('.modal-body').html('Cảm ơn bạn! Chúng tôi sẽ liên hệ lại cho bạn sớm nhất có thể.'+
								'<br/><a class="btn-enroll btn-skin-1" href="index.html">Về trang chủ</a>')
